...
(with-open-file
   (*standard-output* "/dev/null" :direction :output :if-exists :supersede)
      (ql:quickload '(alexandria iterate anaphora)))
(defpackage parmesan
   (:use cl alexandria iterate anaphora)
   (:export :parse-stack :match-stack :get-str :get-ind :ind :str-match :apply-pred :merge-str
      :parse :bounded-area :build-pointer :pretty-error :call-with-context :return-with-context
      :defparser :cat :fail :pass :seq :choice :one-of :none-of :many :sep-many :many+ :sep-many+
      :sep-num :num :par :ret :str :skip :consume-byte :restr :option :between :any-char
      :sym :letter :digit :hex-digit :whitespace))
(in-package parmesan)
